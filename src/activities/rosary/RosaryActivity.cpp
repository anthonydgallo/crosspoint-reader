...
void RosaryActivity::startRosary(RosaryData::DayOfWeek day) {
  startActivityForResult(std::make_unique<RosaryPrayerActivity>(renderer, mappedInput, day),
                         [this](const ActivityResult&) { requestUpdate(); });
}

void RosaryActivity::showMysteryList(RosaryData::DayOfWeek day) {
  startActivityForResult(std::make_unique<RosaryMysteryListActivity>(renderer, mappedInput, day),
                         [this](const ActivityResult&) { requestUpdate(); });
}

void RosaryActivity::showPrayerReference() {
  startActivityForResult(std::make_unique<RosaryPrayerReferenceActivity>(renderer, mappedInput),
                         [this](const ActivityResult&) { requestUpdate(); });
}
...
void RosaryActivity::render(RenderLock&&) {
