...
void ActivityManager::goToOpenApp(const AppManifest& app) {
  if (app.type == "rosary") {
    ...
    replaceActivity(std::make_unique<MinesweeperAppActivity>(renderer, mappedInput, app));
  } else if (app.type == "calculator") {
    replaceActivity(std::make_unique<CalculatorAppActivity>(renderer, mappedInput, app));
    ...
    replaceActivity(std::make_unique<FlashcardAppActivity>(renderer, mappedInput, app));
